...
async def generate_cover(requested_by, title, views, duration, thumbnail):
    ...
    draw.text(
        (190, 670),
        f"ᴛʜɪs ᴍɪsɪᴄ ʙᴏᴛ ʙʏ ᴍʀ ᴀɴᴋɪᴛ ʏᴀᴅᴀᴠ",
        (255, 255, 255),
        font=font,
    )
    img.save("final.png")
...
@Client.on_message(
    commandpro(["/play", "/ytp", "ankit", "Play"])
    & filters.group
    & ~filters.edited
    & ~filters.forwarded
    & ~filters.via_bot
...
async def play(_, message: Message):
    ...
    lel = await message.reply("**ᴡᴀɪᴛɪɴɢ ʙᴀʙʏ...**")
    ...
    if audio:
        ...
        title = file_name
        thumb_name = "https://telegra.ph/file/9de41f60df9976efd418f.jpg"
...
async def pause(_, message: Message):
    ...
    await message.reply_photo(
                             photo="https://telegra.ph/file/9de41f60df9976efd418f.jpg", 
                             caption="**🌷ᴀɴᴋɪᴛ ʏᴀᴅᴀᴠ 🌷ṙ 🤞 Mʋsɩƈ\n💞Nøω 🥀 ▶️ Ƥɑʋsɘɗ 🌷 ...**"
    )
...
@Client.on_message(commandpro(["/skip", "yadav", "/next", "Skip", "Next"]) & other_filters)
@errors
@authorized_users_only
async def skip(_, message: Message):
    ...
    if int(chat_id) not in ACTV_CALL:
        await message.reply_text("**🌷ᴀɴᴋɪᴛ ʏᴀᴅᴀᴠ 🌷ṙ 🤞 ṀṳṠḭḉ 💞\nƝøʈɦɩɳʛ 🔇 🚫 Ƥɭɑyɩɳʛ 🌷 ...**")
    else:
